...
public class InjectionList extends ArrayList<Injection> {
        ...
        if (this.isEmpty()) {
            ...
            if (injection == null) {
                ...
                do {
                    ...
                    int checkDate = newSecondDate.getDayOfYear() - injection.getFirstDate().getDayOfYear();
                    if (checkDate < 28 && checkDate > 84) {
                        System.out.println("The 2nd dose must be 4 to 12 weeks");
                        check = true;
                    }
    ...
    public void searchByStudentName() {
        if (this.isEmpty()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                if (list.get(i).getStudentName().equals(studentName)) {
    ...
    private byte[] serialize(Injection injection) {
    ...
    private Injection deserialize(byte[] data) {
    ...
    private byte[] encryption(Injection injection, PublicKey publicKey) {
        ...
        try {
            ...
            byte[] secretMessageBytes = serialize(injection);
    ...
    private Injection decryption(byte[] encryptedMessageBytes, PrivateKey privateKey) {
        ...
        try {
            ...
            injection = (Injection) deserialize(decryptedMessageBytes);
        } catch (Exception e) {
            System.out.println(e);
        }
        return injection;
    ...
    public void saveEncryption(String fileName, PublicKey publicKey) {
        ArrayList<String> encrypList = new ArrayList<>();
        for (Injection injection : this) {
    ...
    public void readEcryption(String fileName, PrivateKey privateKey) {
        ArrayList<Byte> decrypList = new ArrayList<>();
        try {
            File file = new File(fileName);
            if (file.exists()) {
